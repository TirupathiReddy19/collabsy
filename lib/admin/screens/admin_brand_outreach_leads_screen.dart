...
import '../providers/admin_brand_leads_providers.dart';
...
/// Cross-intern tracking view over `brandLeads` — mirrors
/// `admin_outreach_leads_screen.dart`'s date-range + filter + CSV-export
/// shape exactly. Every brand intern's own tool only shows their own leads
/// (see `brand_intern_home_screen.dart`); this is the only place the full
/// outreach funnel (clicked/signedUp/onboarding complete) is visible
/// across everyone.
class AdminBrandOutreachLeadsScreen extends ConsumerStatefulWidget {
  const AdminBrandOutreachLeadsScreen({super.key});

  @override
  ConsumerState<AdminBrandOutreachLeadsScreen> createState() =>
      _AdminBrandOutreachLeadsScreenState();
}

class _AdminBrandOutreachLeadsScreenState
    extends ConsumerState<AdminBrandOutreachLeadsScreen> {
  ...
  List<BrandLead> _filter(List<BrandLead> leads) {
  ...
  void _exportCsv(List<BrandLead> leads) {
    ...
    downloadCsv(
      'collabsy-brand-outreach-leads-${_dateKey(_start)}-to-${_dateKey(_end)}.csv',
      rows.join('\n'),
    );
  }
  ...
  Widget build(BuildContext context) {
    final leadsAsync = ref.watch(allBrandLeadsProvider);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const AdminTopBar(
          title: 'Brand Outreach Leads',
          subtitle:
              'Brand-intern cold-outreach tracking — clicked, signed up, '
              'onboarding complete',
        ),
        Expanded(
          child: SingleChildScrollView(
            ...
            child: leadsAsync.when(
              ...
              error: (error, stackTrace) => Text(
                "Couldn't load brand leads: $error",
