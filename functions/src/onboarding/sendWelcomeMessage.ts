...
const CREATOR_WELCOME = {
  ...
  body:
    "Your profile is live and ready to go. Explore open campaigns, " +
...
const BRAND_WELCOME = {
  title: "Welcome to Collabsy! 🎉",
  body:
    "Your profile is live and ready to go. Head to Discover to find the " +
 ...
 * new later. */
export const onCreatorApproved = onDocumentUpdated(
  "creatorProfiles/{userId}",
  async (event) => {
    const before = event.data?.before.data();
    const after = event.data?.after.data();
    if (!after || before?.verificationStatus === after.verificationStatus) return;
    if (after.verificationStatus !== "approved") return;

    await sendWelcomeIfNeeded(event.params.userId, "creator", CREATOR_WELCOME);
  }
);

export const onBrandApproved = onDocumentUpdated(
...
export { BRAND_WELCOME, CREATOR_WELCOME, sendWelcomeIfNeeded };
